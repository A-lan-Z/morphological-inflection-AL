...
class Decoder(object):
    ...
    def __call__(self, transducer, src_sentence, src_mask):
        if self.type == Decode.greedy:
            ...
            output, attns, nlls, entropies = decode_fn(
                transducer,
                src_sentence,
                src_mask,
        ...
        elif self.type == Decode.ensemble:
            if isinstance(transducer, Transformer):
                decode_fn = decode_beam_transformer_ensemble
            else:
                raise ValueError("Ensemble decoding is only supported for Transformer models.")
            ...
            raise ValueError
        return_values = (output, nlls, entropies, None if self.skip_attn else attns)
...
def decode_greedy_transformer(
    ...
    finished = None
    nlls_per_instance = torch.zeros(bs, device=DEVICE)
    sequence_lengths = torch.ones(bs, device=DEVICE)
    ...
    for _ in range(max_len):
        ...
        word_logprob_value, word = torch.max(word_logprob, dim=1)  # Get both the value and index
        nlls_per_instance += -word_logprob_value
        output = torch.cat((output, word.view(1, bs)))
        ...
        # Update sequence lengths for non-finished sequences
        sequence_lengths += (~finished).float()

        if finished.all().item():
            break

    # Normalize the NLLs by sequence length
    normalized_nlls = nlls_per_instance / sequence_lengths
    normalized_nlls_list = normalized_nlls.tolist()
    na_list = ["NA"] * len(normalized_nlls_list)
    return output, None, normalized_nlls_list, na_list
...
def decode_beam_transformer(
            ...
            for log_prob, word in zip(topk_log_prob, topk_word):
                ...
                if (word == trg_eos).any():
                    ...
                    for j in batch_idx:
                        ...
                        seq = output[:, j].tolist()
                        log_prob[j] = -1e6
                        finish_beams[j].append((score, seq, cur_len))
    ...
    # Denormalize the log likelihoods
...
def decode_beam_transformer_ensemble(
    ...
    # Instead of returning the best sequence, return all sequences in finish_beams
    # along with their probs.
    all_sequences = []
    all_log_probs = []
    for beams in finish_beams:
        sequences = [seq for _, seq, _ in beams]
        log_probs = [score for score, _, _ in beams]
        all_sequences.append(sequences)
        all_log_probs.append(log_probs)

    return all_sequences, None, all_log_probs, probs
