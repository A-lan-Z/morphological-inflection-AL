...
def process_directory(directory, accuracy_output_file, difficulty_output_file, dist_output_file):
    ...
    for filename in filenames:
        if filename.endswith(".tsv") and '.decode.test_' in filename:
            ...
            # difficulty_file_path = f"../experiments/ensemble_entropy/{language}/{language}_difficulty_25.tsv"
    ...
    with open(accuracy_output_file, 'w', newline='', encoding='utf-8') as f:
        ...
        writer.writerow(['test result name', 'overall', 'both', 'lemma', 'feats', 'neither'])
...
if __name__ == "__main__":
    ...
    #     for seed in seeds:
    #         for experiment in experiments:
    #             directory_path = f"../experiments/ensemble_entropy/{language.strip()}/separate/{seed.strip()}"
    #
    #             suffix = get_filename_suffix(experiment.strip())
    #             accuracy_output_file_path = f"../experiments/ensemble_entropy/{language.strip()}/separate/{seed.strip()}_{language.strip()}_accuracies{suffix}"
    #             difficulty_output_file_path = f"../experiments/ensemble_entropy/{language.strip()}/separate/{seed.strip()}_{language.strip()}_difficulties{suffix}"
    #             dist_output_file_path = f"../experiments/ensemble_entropy/{language.strip()}/separate/{seed.strip()}_{language.strip()}_dist{suffix}"
    #
    #             process_directory(directory_path, accuracy_output_file_path, difficulty_output_file_path,
    #                               dist_output_file_path)
